...
### [2026-09-18 15:45] Milestone 10: Complete Elimination of Demo Outcomes/Habits & Task Completion Button Fix
- **Problem**:
  1. Previously seeded demo outcomes (`task-1`..`task-6` and hundreds of `task-hist-*` generated by the seeder loop, totaling "621 done") remained visible in the user's tasks list and "Completed Today" card.
  2. The task completion button (`.btn-check-task`) failed with an unhandled exception in `tasks.js` and did not trigger re-rendering in `home.js`.
     - In `tasks.js`: `window.ParticleSystem.taskComplete(rect.left + rect.width / 2, ...)` passed numbers to `taskComplete(taskCardEl)`, causing `(number).classList.add` to throw `TypeError: Cannot read properties of undefined (reading 'add')`, terminating execution before `renderTasksView` was called.
     - In `home.js`: Toggling `.btn-check-task` never invoked `renderHomeView()`, leaving the UI stale.
  3. Pre-seeded habits from `DEFAULT_HABITS` and sample routine steps (`m-1`..`m-4`, `n-1`..`n-4`) were cluttering the interface instead of showing only user-added items.
- **Modifications Made**:
  1. **Resilient Particle System (`js/visuals/particles.js`)**:
     - Added `burstAt(cx, cy, color, count)` to cleanly emit particle bursts at window coordinates.
     - Refactored `burst(originEl, color, count)` to safely extract bounding box coordinates.
     - Upgraded `taskComplete(target, y, color)` to handle both HTMLElement instances and numeric `(x, y)` coordinate arguments seamlessly, wrapped in try-catch so it never throws.
     - Guarded global `window.ParticleSystem` assignment for headless/Node environments.
  2. **Task Completion Fix in Views (`js/views/tasks.js`, `js/views/home.js`)**:
     - In `tasks.js`: Fixed `.btn-check-task` to pass the card element to `ParticleSystem.taskComplete`, wrapped effects in try-catch, and guaranteed `renderTasksView(container, navigate)` executes.
     - In `tasks.js`: Fixed `completedTodayCount` to accurately compute tasks completed on the active date (`completedAt.startsWith(todayStr)`).
     - In `home.js`: Added missing `renderHomeView(container, navigate)` call to `.btn-check-task` listener so task completion toggles update the home dashboard immediately.
  3. **Purge Demo Data in Store Engine (`js/store/db.js`)**:
     - Created `isDemoTask(t)` matching `DEMO_TASK_IDS`, prefixes (`task-hist-`, `task-seed-`, `task-demo-`), and all known demo task titles.
     - In `loadState()`: Unconditionally stripped all demo tasks, demo habits (`DEMO_HABIT_IDS`), orphaned habit records, and demo routine steps (`DEMO_ROUTINE_STEP_IDS`).
     - In `createSeedState()`: Completely deleted `historicalTasks` generation loop and `tasks.push(...historicalTasks)`. Seeder now initializes `tasks = []`, `habits = []`, `habitRecords = []`, and empty routine step arrays while preserving background focus sessions and sleep records for the annual dot matrix and circadian charts.
     - In `clearDemoTasks()`, `clearDemoHabits()`, `clearDemoRoutines()`: Upgraded to thoroughly purge all demo data.
  4. **Fallback UI for Empty Routines & Habits (`js/views/sleep.js`, `js/views/habits.js`)**:
     - In `sleep.js`: Added a clean dashed card fallback when a protocol has 0 steps ("No custom routine steps added yet. Click 'Edit Protocol' above to configure your steps.").
     - In `habits.js`: Added an empty state fallback in `renderTodayDeckSection`.
  5. **Cache Busting**:
     - Bumped query string versioning to `?v=14.0` in `index.html`.
  6. **Automated Verification**:
     - Executed unit tests verifying `isDemoTask`, store methods, toggle completed persistence, and ParticleSystem resilience.
...
- **Reasoning**: Guaranteed that the user has an immaculate, distraction-free cockpit displaying strictly their own tasks, habits, and protocols, with fully functional, tactile completion toggles across all views.
